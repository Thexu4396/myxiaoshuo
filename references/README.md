...
本目录存放 AI Story Craft 的所有参考资料，供 skills 和 scripts 在运行时读取。
...
| `review-schema.md` | 审查输出格式定义 | `story-reviewer` 读取 |
| `index/` | 元数据索引（loading-map、gap-register） | 人工参考 |
| `outlining/` | 大纲相关参考 | `plot-planner` 读取 |
| `review/` | 审查相关参考 | `story-reviewer` 读取 |
...
`story-architect -> plot-planner -> chapter-writer -> story-reviewer` 的完整 reference 消费路径见 `index/reference-loading-map.md`。
